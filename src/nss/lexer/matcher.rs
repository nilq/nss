...
macro_rules! token {
    ...
    ($tokenizer:expr, $token_type:expr, $accum:expr) => {{
        let tokenizer = $tokenizer as &$crate::nss::lexer::tokenizer::Tokenizer<'t>;
        let token_type = $token_type as $crate::nss::lexer::token::TokenType;
...
impl<'t> Matcher<'t> for CommentMatcher {
    fn try_match(&self, tokenizer: &mut Tokenizer<'t>) -> Result<Option<Token>, ()> {
        if tokenizer.peek_range(2).unwrap_or_else(String::new) == "--" {
            while !tokenizer.is_end() && tokenizer.peek() != Some('\n') {
                tokenizer.advance()
            }
